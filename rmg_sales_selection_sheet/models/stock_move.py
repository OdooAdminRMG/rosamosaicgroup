...
class StockMoveLine(models.Model):
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        """
        rtn = super(StockMoveLine, self)._prepare_move_line_vals(quantity, reserved_quant)

        for line in self.picking_id.sale_id.order_line.filtered(
                lambda line: line.display_type == False and line.rmg_sale_id):
            bom_line_ids = self.env['mrp.bom'].search(
                ['|', '|', ('byproduct_ids.product_id', '=', self.product_id.id),
                 ('product_id', '=', line.product_id.id), '&',
                 ('product_id', '=', False),
                 ('product_tmpl_id', '=', line.product_id.product_tmpl_id.id)]).bom_line_ids
