...
class Project(models.Model):
	...
	def map_tasks(self, new_project_id):
		...
		old_task_ids = self.env['project.task'].with_context(active_test=False).search([('project_id', '=', self.id)], order='parent_id')
		# Get task_id sequence for task and it's related depend_on_ids tasks
		old_depend_on_task_ids = {
			task.task_id: task.depend_on_ids.mapped('task_id') for task in old_task_ids if task.depend_on_ids}
