...
class ReplenishSources(models.Model):
    ...
    _description = "The records of this model will be either created from Sale Order or from Manufacturing Order." \
    ...
    mo_origin = fields.Char(string=_('Manufacturing Origin'), )

    so_id = fields.Many2one(related='so_line_id.order_id',
                            string=_("Sale Order"),
                            store=True,
                            )
    job_name = fields.Char(compute='_compute_replenish_sources_data',
                           string=_("Job Name"),
                           store=True,
    ...
    product_uom_qty = fields.Float(compute='_compute_replenish_sources_data',
                                   string=_("Quantity"),
                                   store=True,
                                   )
    price_unit = fields.Float(compute='_compute_replenish_sources_data',
                              string=_("Unit Price"),
                              store=True,
                              )
    ...
    requested_on = fields.Datetime(string=_('Requested On'),
    ...
    def create_replenish_sources_history_for_existing_po(self):
        ...
        for po in self.env['purchase.order'].search([]):
            for so_rs in self.env['sale.order.line'].search(
                    [
